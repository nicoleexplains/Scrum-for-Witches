...
import { Task, ChecklistItem, DailyStandup } from '../types';
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700">
            ...
            <div className="space-y-4">
              ...
              <TextareaField name="action" value={editedTask.action} onChange={(e) => handleInputChange(e as React.ChangeEvent<HTMLInputElement>)} placeholder="e.g., research planetary hours for Jupiter" label="I want to..." />
              <TextareaField name="goal" value={editedTask.goal} onChange={(e) => handleInputChange(e as React.ChangeEvent<HTMLInputElement>)} placeholder="e.g., find the best time for my prosperity spell" label="So that I can..." />
            </div>
          </div>
            ...
            <div className="space-y-2">
              {editedTask.definitionOfDone.map(item => (
                <div key={item.id} className="flex items-center group">
                  ...
                  />
                  <span className={`ml-3 text-slate-300 ${item.completed ? 'line-through text-slate-500' : ''}`}>{item.text}</span>
